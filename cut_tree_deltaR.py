...
class CutFunction:
    ...
    def generateCppFunc_CutByDeltaR(self) -> str:
        if self.pfCand_entry != 'pfCand_DeltaR':
            return f"""
            std::vector<{self.pfCand_entryDatatype}> {self.functionName} (const ROOT::VecOps::RVec<{self.pfCand_entryDatatype}>& {self.pfCand_entry}, const ROOT::VecOps::RVec<float>& pfCand_DeltaR){{
...
cutFunctions = { 
    ...
    CutFunction("pfCand_PuppiWeightNoLep", "int32_t", "pfCand_puppiWeightNoLep"),
...
files = glob.glob("/ceph/aissac/ntuple_for_graphs/prod_2018_v2_processed_v4/trimmed_500000_and_added_deltaPhiEtaR" + "/**/*.root", recursive=True)
treename = "taus"
n = 200000

outputFolder = path.join('/ceph/aissac/ntuple_for_graphs/prod_2018_v2_processed_v4', f'trimmed_{n}_and_cut_deltaR_bigger_0Point5')
